...
grep -q 'ReviveTLS-M8-WM6-ARMV4I' "${workflow}"
...
for required_source in app/main.cpp app/ui.cpp common/log.cpp feeds/feed.cpp mail/imap.cpp mail/smtp.cpp net/socket.cpp net/tls.cpp net/http.cpp; do
...
for required_http_control in \
    ReviveHttpParseUrl \
    ReviveHttpGet \
    'Accept-Encoding: identity' \
    'Transfer-Encoding' \
    'https://www.google.com/robots.txt'; do
    if ! grep -Fq "${required_http_control}" revivece/net/http.cpp revivece/app/ui.cpp; then
        echo "ERROR: M7 HTTPS control is missing: ${required_http_control}." >&2
        exit 1
    fi
done
...
for required_message_control in \
    ...
    REVIVE_IMAP_INITIAL_BODY_BYTES; do
    if ! grep -Fq "${required_message_control}" revivece/mail/imap.cpp revivece/mail/imap.h; then
